...
def ReadFiles(mRNA_to_analyse,widths_to_analyse,soma_bins,dend_bins,bin_size,channel_3_mRNA,lengths):
     ...
     for width in widths_to_analyse:
        ...
        for mRNA in mRNA_to_analyse:
            ...
            rfolder = os.path.abspath(os.getcwd()) + "../"
            folder = rfolder+"/{}/cells/".format(mRNA)
    ...
    for cell in od.keys():
        ...
        for unit in od[cell].keys():
            ...
            if all_stats.shape[0] != 0:
                ...
                sw_name = SlidingWindowMean(all_stats,5 // 0.24,-1,"{0}_{1}_{2}".format("no channel",cell,unit)) # binning based on lenght  
            else:
                # breakpoint()
...
def SlidingWindowMean(arr,window_len,num_col=-1,name=None):
   
    arr = SortPunctas(arr,num_col)
    # v = sliding_window_view(arr, window_len)
    if (arr[:,-1] > 0).any():
         breakpoint()
    return []

def myround(x, num_col = 0,prec=2, base=.05):
  return round(base * round(float(x[num_col])/base),prec)
...
def GetRNAPunctaStat(puncta):
    ...
    stat4 = med
    stat5 = area
    puncta_stat = np.array([stat1,stat2,stat3,stat4,stat5,1,dfs])
        ...
        for mrna in dend_sum_norm_distribution_dict[width].keys():
            ...
            for ldx,l in enumerate(lengths_to_analyse):
               ...
               if dend_sum_norm_distribution_dict[width][mrna][l1].shape[0] >0:
                   total_puncta_dist[width][mrna][l1] = dend_sum_norm_distribution_dict[width][mrna][l1][:,:,5].sum(axis=0)
               else:
        ...
        for mrna in mRNA_to_analyse:
            ...
            for l in lengths_to_analyse:
                ...
                if in_set == 1:
                    ...
                    means[0] = dend_sum_norm_distribution_dict[width][mrna][l][:,:,stat_no].mean(axis=0)[1:xs.shape[0]+1]
                    stds[0] = dend_sum_norm_distribution_dict[width][mrna][l][:,:,stat_no].std(axis=0)[1:xs.shape[0]+1]
                    
                    means[1] = dend_sum_norm_distribution_dict[width][channel_3_mRNA][l][:,:,stat_no].mean(axis=0)[1:xs.shape[0]+1]
                    stds[1] = dend_sum_norm_distribution_dict[width][channel_3_mRNA][l][:,:,stat_no].std(axis=0)[1:xs.shape[0]+1]
                    # breakpoint()
                    MAP2_norm_means = means/MAP2_mean[width][l][1:xs.shape[0]+1]
                    MAP2_norm_stds = ((stds/means + (MAP2_std[width][l1]/MAP2_mean[width][l])[1:xs.shape[0]+1]) * MAP2_norm_means)
                    MAP2_norm_stds[0] = MAP2_norm_stds[0]/np.sqrt(dend_sum_norm_distribution_dict[width][mrna][l][:,:,stat_no].shape[0])
                    MAP2_norm_stds[1] = MAP2_norm_stds[1]/np.sqrt(dend_sum_norm_distribution_dict[width][channel_3_mRNA][l][:,:,stat_no].shape[0])
                ...
                pw.PlotBinnedStats(np.asarray([xs,xs]), MAP2_norm_means, MAP2_norm_stds,norm_density_mean, labs, x_lab, y_lab, y_lab_norm,plot_colors,title, op_folder+file_prefix+"_norm_{0}_{1}_{2}_len_{3}".format(stats_list[stat_no],mrna,width,l)\
                                  ,bin_size,save_it = save_it,fit_exp=0,in_set=in_set,set_axis_label=ax_label)
                # breakpoint()
                pw.PlotBinnedStats(np.asarray([xs,xs]), norm_density_mean, norm_density_std, norm_density_mean, labs, x_lab, y_lab, y_lab_norm, plot_colors,title, op_folder+file_prefix+"_norm__with_fit_{0}_{1}_{2}_len_{3}".format(stats_list[stat_no],mrna,width,l)\
